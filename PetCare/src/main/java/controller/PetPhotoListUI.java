...
public class PetPhotoListUI extends JFrame {
    private static final long serialVersionUID = 1L;
    private Pet pet;
    private JPanel contentPane;
    private JTable table;

    public PetPhotoListUI() {
        this(createDemoPet());
    }

    public PetPhotoListUI(Pet pet) {
        this.pet = pet;
        initialize();
        loadData();
    }
    ...
    private void initialize() {
        ...
        JLabel menuLabel = new JLabel("成長相簿");
        ...
        JLabel breadcrumbLabel = new JLabel("我的寵物 / 寵物詳細 / 成長相簿");
        ...
        JLabel titleLabel = new JLabel("成長相簿");
        ...
        JLabel subtitleLabel = new JLabel("寵物：" + ModernUI.safe(pet.getName()) + " 的成長相簿");
        ...
        editButton.addActionListener(e -> { int id = CrudHelper.selectedId(table); if (id > 0 && CrudHelper.edit("photos", id)) loadData(); });
    ...
    private void loadData() {
        ...
        List<PetPhoto> records = service.findByPetId(pet.getId());
        DefaultTableModel model = createModel();
        for (PetPhoto r : records) {
            model.addRow(new Object[] { r.getId(), r.getTitle(), r.getPhotoPath(), r.getPhotoDate(), r.getNote() });
        }
    ...
    private void openAddUI() {
        AddPetPhotoUI ui = new AddPetPhotoUI(pet);
        ui.setVisible(true);
        dispose();
    }
