...
public class VaccineRecordListUI extends JFrame {
    private static final long serialVersionUID = 1L;
    private Pet pet;
    private JPanel contentPane;
    private JTable table;

    public VaccineRecordListUI() {
        this(createDemoPet());
    }

    public VaccineRecordListUI(Pet pet) {
        this.pet = pet;
        initialize();
        loadData();
    }
    ...
    private void initialize() {
        ...
        JLabel menuLabel = new JLabel("疫苗紀錄");
        ...
        JLabel breadcrumbLabel = new JLabel("我的寵物 / 寵物詳細 / 疫苗紀錄");
        ...
        JLabel titleLabel = new JLabel("疫苗紀錄");
        ...
        JLabel subtitleLabel = new JLabel("寵物：" + ModernUI.safe(pet.getName()) + " 的疫苗紀錄");
        ...
        editButton.addActionListener(e -> { int id = CrudHelper.selectedId(table); if (id > 0 && CrudHelper.edit("vaccines", id)) loadData(); });
    ...
    private DefaultTableModel createModel() {
        ...
        model.addColumn("ID");
        model.addColumn("疫苗名稱");
        model.addColumn("施打日期");
        model.addColumn("下次日期");
        model.addColumn("醫院");
        model.addColumn("備註");
        return model;
    }
    ...
    private void loadData() {
        ...
        List<VaccineRecord> records = service.findByPetId(pet.getId());
        DefaultTableModel model = createModel();
        for (VaccineRecord r : records) {
            model.addRow(new Object[] { r.getId(), r.getVaccineName(), r.getVaccineDate(), r.getNextDate(), r.getHospital(), r.getNote() });
        }
    ...
    private void openAddUI() {
        AddVaccineRecordUI ui = new AddVaccineRecordUI(pet);
        ui.setVisible(true);
        dispose();
    }
