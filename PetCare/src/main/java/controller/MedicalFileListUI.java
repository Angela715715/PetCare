...
public class MedicalFileListUI extends JFrame {
    private static final long serialVersionUID = 1L;
    private Pet pet;
    private JPanel contentPane;
    private JTable table;

    public MedicalFileListUI() {
        this(createDemoPet());
    }

    public MedicalFileListUI(Pet pet) {
        this.pet = pet;
        initialize();
        loadData();
    }
    ...
    private void initialize() {
        ...
        JLabel menuLabel = new JLabel("醫療文件");
        ...
        JLabel breadcrumbLabel = new JLabel("我的寵物 / 寵物詳細 / 醫療文件");
        ...
        JLabel titleLabel = new JLabel("醫療文件");
        ...
        JLabel subtitleLabel = new JLabel("寵物：" + ModernUI.safe(pet.getName()) + " 的醫療文件");
        ...
        editButton.addActionListener(e -> { int id = CrudHelper.selectedId(table); if (id > 0 && CrudHelper.edit("files", id)) loadData(); });
    ...
    private void loadData() {
        ...
        List<MedicalFile> records = service.findByPetId(pet.getId());
        DefaultTableModel model = createModel();
        for (MedicalFile r : records) {
            model.addRow(new Object[] { r.getId(), r.getFileName(), r.getFilePath(), r.getFileType(), r.getUploadDate(), r.getNote() });
        }
    ...
    private void openAddUI() {
        AddMedicalFileUI ui = new AddMedicalFileUI(pet);
        ui.setVisible(true);
        dispose();
    }
